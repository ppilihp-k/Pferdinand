...
class Pferdinand:
    ...
    def __handle_waiting_state(self, event: Event) -> int:
        if Event.TIME_TICK == event.event_id():
            ...
            print(
                f'Active at {self.__active_at}, current: {timestamp}'
            )
            if timestamp.hours() == self.__active_at.hours() and timestamp.minutes() == self.__active_at.minutes() and abs(timestamp.seconds() - self.__active_at.seconds()) < 5:
